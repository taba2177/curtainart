...
<section class="bg-gray-50 dark:bg-gray-900 py-12">
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 lg:gap-8">
            ...
            <div class="rounded-xl overflow-hidden shadow-md dark:shadow-gray-700 flex flex-col bg-white dark:bg-gray-800 wow fadeInUp" data-wow-delay="{{ ($loop->index + 1) * 0.1 }}s">
                <a href="{{ $post->url }}">
                    <div class="relative">
                        <img class="w-full object-cover object-center aspect-video" src="{{ asset($post->image?->url ?? $post->getRandomImage()) }}" alt="{{ $post->title }}">
                        <div class="absolute inset-0 bg-black opacity-20 hover:opacity-10 transition-opacity duration-300"></div>
                ...
                <div class="px-4 py-3 sm:px-6 sm:py-4 flex-grow flex flex-col justify-between">
                    <div>
                        <a href="{{ $post->url }}" class="block text-lg font-semibold text-gray-900 dark:text-gray-50 hover:text-primary-500 dark:hover:text-primary-400 transition-colors mb-2">
                            {{ $post->title }}
                        </a>
                    ...
                    <div class="flex items-center justify-between mt-4">
                        <span class="text-gray-500 dark:text-gray-400 text-sm flex items-center">
                            <x-heroicon-o-clock class="h-4 w-4 mr-1 rtl:ml-1" />
